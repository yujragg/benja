...
def main():
    ...
    if uploaded_file is not None:
        sheet_name = st.text_input("Ingrese el nombre de la hoja")
        col_range = st.text_input("Ingrese el rango de columnas (ej. A:D)")
        start_row = st.text_input("Ingrese la fila inicial")
        ...
        if st.button("Procesar Datos"):
            ...
            if df is not None:
                ...
                chart_type = st.selectbox("¿Qué tipo de gráfico desea generar?", ["Torta", "Barras"])
                column = st.text_input("Ingrese el nombre de la columna para graficar")
                max_data = st.number_input("Ingrese la cantidad máxima de datos a graficar", min_value=1, max_value=100, value=10)
